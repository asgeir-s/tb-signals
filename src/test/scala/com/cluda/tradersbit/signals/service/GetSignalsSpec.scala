...
import com.cluda.tradersbit.signals.{DatabaseUtilBlockingForTests, TestData}
import com.cluda.tradersbit.signals.model.{Signal, SignalJsonProtocol}
...
class GetSignalsSpec extends TestService {
  ...
  it should "responds withe all the signals for the given stream with the given ID" in {
    Get("/streams/" + streamID + "/signals").addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
  ...
  it should "responds withe all cloased signals when 'onlyClosed' is true" in {
    Get("/streams/" + streamID + "/signals?onlyClosed=true").addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
      import SignalJsonProtocol._
      import spray.json._
      val signals =  responseAs[String].parseJson.convertTo[List[Signal]]
  ...
  it should "responds withe an empty array when the their is no stream with the given streamID" in {
    Get("/streams/" + "notexisting" + "/signals").addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
  ...
  it should "on status request responds the last signal stream with the given ID" in {
    Get("/streams/" + streamID + "/status").addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
      import SignalJsonProtocol._
      import spray.json._
      val signals =  responseAs[String].parseJson.convertTo[List[Signal]]
  ...
  it should "be possible to get all signals with id higher then a specified id" in {
    Get("/streams/" + streamID + "/signals?fromId=5").addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
      import SignalJsonProtocol._
      import spray.json._
      val signals =  responseAs[String].parseJson.convertTo[List[Signal]]
  ...
  it should "be possible to get all signals after a specified timestamp" in {
    Get("/streams/" + streamID + "/signals?beforeTime=" + (TestData.timestamp - 40000)).addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
      import SignalJsonProtocol._
      import spray.json._
      val signals =  responseAs[String].parseJson.convertTo[List[Signal]]
    ...
    Get("/streams/" + streamID + "/signals?afterTime=" + (TestData.timestamp - 40001)).addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
      import SignalJsonProtocol._
      import spray.json._
      val signals =  responseAs[String].parseJson.convertTo[List[Signal]]
  ...
  it should "be possible to get all signals between two timestamps" in {
    Get("/streams/" + streamID + "/signals?afterTime=" + (TestData.timestamp - 110000) + "&beforeTime=" + (TestData.timestamp - 50000)).addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
      import SignalJsonProtocol._
      import spray.json._
      val signals =  responseAs[String].parseJson.convertTo[List[Signal]]
  ...
  it should "be possible to get the last n signals" in {
    Get("/streams/" + streamID + "/signals?lastN=" + 6).addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
      import SignalJsonProtocol._
      import spray.json._
      val signals =  responseAs[String].parseJson.convertTo[List[Signal]]
    ...
    Get("/streams/" + streamID + "/signals?lastN=" + 10).addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
      import SignalJsonProtocol._
      import spray.json._
      val signals =  responseAs[String].parseJson.convertTo[List[Signal]]
    ...
    Get("/streams/" + streamID + "/signals?lastN=" + 1).addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
      import SignalJsonProtocol._
      import spray.json._
      val signals =  responseAs[String].parseJson.convertTo[List[Signal]]
  ...
  it should "be possible to get all signals between to id's" in {
    Get("/streams/" + streamID + "/signals?fromId=" + 4 + "&toId=" + 9).addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
      import SignalJsonProtocol._
      import spray.json._
      val signals =  responseAs[String].parseJson.convertTo[List[Signal]]
  ...
  it should "be possible to get all signals before a specified id's" in {
    Get("/streams/" + streamID + "/signals?toId=" + 9).addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
      import SignalJsonProtocol._
      import spray.json._
      val signals =  responseAs[String].parseJson.convertTo[List[Signal]]
  ...
  it should "be possible to get all signals before a time" in {
    Get("/streams/" + streamID + "/signals?beforeTime=" + (TestData.timestamp - 80000l)).addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe OK
      import SignalJsonProtocol._
      import spray.json._
      val signals =  responseAs[String].parseJson.convertTo[List[Signal]]
  ...
  it should "return a error when a invalid combination of parameters are used (lastN should always be alone)" in {
    ...
    Get("/streams/" + streamID + "/signals?lastN=" + 1 + "&afterTime=" + (TestData.timestamp - 80000l) +
      "&beforeTime=" + (TestData.timestamp - 80000l)).addHeader(authorizationHeader).addHeader(globalRequestIDHeader) ~> routes ~> check {
      status shouldBe BadRequest
      val responds = responseAs[String]
      assert(responds.contains("invalid combination of parameters"))
    }
